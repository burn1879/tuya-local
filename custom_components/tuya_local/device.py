...
class TuyaGatewayDevice(object):
    def __init__(
        self,
        name,
        dev_id,
        ...
        """
        Represents a Tuya-Gateway device.
        ...
        try:
            async for poll in self.async_receive():
                if type(poll) is dict:
                    ...
                    try:
                        if "data" in poll:
                            poll_data = poll.pop("data")
                            device_cid = poll_data.pop("cid", None)
                        elif "cid" in poll:
                            device_cid = poll.pop("cid", None)
                        else:
                            raise KeyError
    ...
    async def async_receive(self):
        """Receive messages from a persistent connection asynchronously."""
        pingtime = time() + self._CACHE_TIMEOUT
...
class TuyaSubDevice(object):
    def __init__(self, name, dev_id, parent, hass: HomeAssistant):
        ...
        Args:
            dev_id (str): The device id.
            parent (TuyaGatewayDevice): The parent.
            hass (HomeAssistant): The Home Assistant instance.
        """
        self._name = name
        self._parent = parent
    ...
    async def async_refresh(self):
        while self._parent.is_busy():
            await asyncio.sleep(2)

        _LOGGER.debug("Refreshing device state for %s", self.name)
        self._parent.set_busy(True)
        await self._retry_on_failed_connection(
            lambda: self._refresh_cached_state(),
            f"Failed to refresh device state for {self.name}.",
        )
    ...
    async def _retry_on_failed_connection(self, func, error_message):
        if self._api_protocol_version_index is None:
            await self._rotate_api_protocol_version()
        auto = (self._parent._protocol_configured == "auto") and (
            not self._api_protocol_working
        )
        connections = (
            self._AUTO_CONNECTION_ATTEMPTS
    ...
    async def _rotate_api_protocol_version(self):
        if self._api_protocol_version_index is None:
            try:
                self._api_protocol_version_index = API_PROTOCOL_VERSIONS.index(
                    self._parent._protocol_configured
                )
            except ValueError:
                self._api_protocol_version_index = 0

        # only rotate if parent configured as auto
        elif self._parent._protocol_configured == "auto":
            self._api_protocol_version_index += 1
...
def setup_device(hass: HomeAssistant, config: dict):
    ...
    if config.get(CONF_IS_GATEWAY):
        device = TuyaGatewayDevice(
            config[CONF_NAME],
    ...
    elif config.get(CONF_PARENT_GATEWAY) != "None":
        parent = hass.data[DOMAIN][config.get(CONF_PARENT_GATEWAY)]
